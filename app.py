...
async def fetch_json(session, url):
    """Простая функция для получения JSON с обработкой ошибок"""
    try:
...
async def get_prices():
    """Получает цены с Bybit и Bitget, используя Google Public DNS (8.8.8.8)"""
    # Создаём connector с явным указанием DNS
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        data = await fetch_json(session, bybit_url)
        ...
        data = await fetch_json(session, bitget_url)
        ...
        if not prices:
            logging.warning("No prices were fetched from exchanges.")
        else:
            logging.info(f"Fetched {len(prices)} prices from exchanges.")
        return prices
...
async def cmd_start(message: types.Message):
    await message.answer(
        f"🤖 **Арбитражный сканер на Render.com (v2)**\n\n"
        f"Bybit ↔ Bitget\n"
...
async def cmd_status(message: types.Message):
    await message.answer("🔍 Сканирую...", reply_markup=refresh_btn)
    prices = await get_prices()
    if not prices:
        await message.answer("❌ Нет данных с бирж. Проверьте логи Render.", reply_markup=refresh_btn)
        return
    
    results = analyze(prices)
    if not results:
        await message.answer(f"✅ Отклонений нет. (Получено цен: {len(prices)})", reply_markup=refresh_btn)
    else:
        text = f"📊 **Найдено: {len(results)}**\n\n"
        for r in results[:10]:
            text += f"**{r['coin']}**: {r['deviation']:.2f}%\n"
            text += f"Bybit: {r['bybit']:.6f}\n"
            text += f"Bitget: {r['bitget']:.6f}\n\n"
...
async def cmd_threshold(message: types.Message):
    ...
    try:
        ...
        await message.answer("❌ /threshold 0.7", reply_markup=refresh_btn)
        ...
        try:
            ...
            if prices:
                ...
                for r in results:
                    ...
                    logging.info(f"Signal sent for {r['coin']}: {r['deviation']:.2f}%")
        except Exception as e:
...
async def on_startup(app):
    ...
    logging.info("Bot started and background scanner is running")
